...
3. 성공 시 200 + 새 토큰을 캐시에 반영, 실패 시 422 + 비즈뿌리오 원문 사유 + result_code
...
| 422 | Unprocessable Entity | 자격증명 미설정(계정/비밀번호 공란) 또는 비즈뿌리오 인증 실패 — `errors.result_code` 에 비즈뿌리오 응답 결과코드(있으면) 동반. 비즈뿌리오 서버 연결 자체가 실패(타임아웃·DNS 등)한 경우도 422(연결 실패 안내 메시지, `result_code` 없음) |
...
실패 시 `BizppurioApiException` 을 그대로 422 로 변환합니다 — 비즈뿌리오 응답에 `description` 필드가 있으면 그 원문 사유를 담은 메시지(`token_issue_failed_with_reason`)를, 없으면 고정 메시지(`token_issue_failed`)를 반환합니다. 응답 `errors.result_code` 는 비즈뿌리오 결과코드(예: `3007`)이며, HTTP 전송 자체가 실패한 경우 `null` 입니다.
